...
class GradeFlowApp:
    def __init__(self, root):
        ...
        title = tk.Label(root, text="Калькулятор среднего балла", font=("Arial", 16, "bold"))
        title.pack(pady=15)

        count_frame = tk.Frame(root)
        count_frame.pack(pady=5)

        tk.Label(count_frame, text="Количество предметов:", font=("Arial", 11)).pack(
            side=tk.LEFT, padx=5
        )

        self.count_var = tk.StringVar(value="3")
        self.count_entry = tk.Entry(
            count_frame, textvariable=self.count_var, width=5, font=("Arial", 11)
        )
    ...
    def create_fields(self):
        ...
        tk.Label(
            header_frame, text="Предмет", font=("Arial", 10, "bold"), width=25, anchor="w"
        ).pack(side=tk.LEFT, padx=5)
        tk.Label(
            header_frame, text="Оценка (1-5)", font=("Arial", 10, "bold"), width=10, anchor="w"
        ).pack(side=tk.LEFT, padx=5)
    ...
    def calculate(self):
        ...
        for i, (subj_entry, grade_entry) in enumerate(
            zip(self.subject_entries, self.grade_entries)
        ):
        ...
        self.result_label.config(
            text=f"Средний балл: {avg:.2f} | {letter}", fg=color, font=("Arial", 14, "bold")
        )
